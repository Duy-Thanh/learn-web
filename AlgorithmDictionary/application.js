...
const API_getCurrentUser = async () => {
    ...
    try {
        ...
        try {
            const { username, email, permission, id, status, fullname } = responseData;
...
/**
 * API_DisplayAlgorithmsBySearchQuery() - Display algorithms
 * 
 * @param search_param: Search query
 * 
 * @return None
 */
const API_DisplayAlgorithmsBySearchQuery = async (search_param) => {
    const endpoint = "/api/get_algorithms_by_search_term";
    const queryParams = { query: search_param };
 ...
 * API_UpdateUser()
 * 
 * @param Long id, String username, String password, String fullname, String email
 * 
 * @return None
 */
const API_UpdateUser = async (id, username, password, fullname, email) => {
    const endpoint = "/api/update_user";
    const queryParams = { id: id, username: username, password: password, fullname: fullname, email: email};

    try {
        var responseData = await getRequest(endpoint, queryParams);

        if (responseData.error || responseData.errorDetails) {
            alert(`Failed to complete update user informations. Please try again later`);
